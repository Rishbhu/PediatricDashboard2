...
st.markdown("""
<style>
/* Hide Streamlit chrome; tighten layout */
...
:root{
  ...
  /* BAR CHART = ALL REDS */
  --bar1:#FCA5A5; /* light red */
  --bar2:#F87171; /* red */
  --bar3:#EF4444; /* strong red */
  --bar4:#DC2626; /* deep red */

  /* Donut */
  --donut1:#F97362; /* coral */
  --donut2:#22D3EE; /* cyan */
  --donut3:#38BDF8; /* sky */
  --donut4:#93C5FD; /* light sky */
}
...
.side h3{
  ...
  letter-spacing:.14em;
  color: var(--heading);
  font-weight:1000; font-size:22px; line-height:1.1;
  white-space:nowrap;                 /* keep single line */
}
.side p{ margin:2px 0 0; font-weight:800; color: var(--ink); }

/* Top band with patient KV */
.topband{
  background: var(--band);
  border:1px solid var(--band-border);
  border-radius:12px;
...
.kv .label{
  font-size:14px; font-weight:1000; letter-spacing:.12em; color: var(--heading);
  white-space:nowrap;
}
.kv .val{margin-top:4px; font-size:18px; font-weight:900; color:var(--ink);}
.baby{
  width:46px;height:46px;border-radius:999px;border:2px solid var(--band-border);
  display:flex;align-items:center;justify-content:center;background:#0b1a30;font-size:22px;
}

/* Headings (kept on one line) */
...
/* Shunt : Weight pill */
.pillbar{
  height:44px;border-radius:999px;background:linear-gradient(90deg, #ef4444 0%, #fb923c 40%, #22c55e 100%);
  display:flex;align-items:center;justify-content:center;border:1px solid #1f2e4a;
}
...
def arc_path(cx, cy, r, start_deg, end_deg):
    start = math.radians(start_deg); end = math.radians(end_deg)
...
def bar_svg(counts)->str:
    # counts = [Downs, Turner, DiGeorge, Williams]
    W, H = 320, 190
...
def donut_svg(segments)->str:
    ...
    for label, pct, color in segments:
        ...
        rx = (R+38)*math.cos(mid) + cx; ry = (R+38)*math.sin(mid) + cy
...
with main:
    ...
    <div class="topband">
      <div class="kvgrid">
        <div class="kv"><div class="label">PATIENT&nbsp;ID</div><div class="val muted">123456789</div></div>
        <div class="kv"><div class="label">DOB</div><div class="val muted">10/01/2025</div></div>
        <div class="kv"><div class="label">GENDER</div><div class="val muted">F</div></div>
        <div class="kv"><div class="label">RACE/ETHNICITY</div><div class="val muted">Hispanic/Latino</div></div>
        <div class="kv"><div class="label">WEIGHT</div><div class="val muted"># kg</div></div>
        <div class="kv"><div class="label">GEST&nbsp;AGE</div><div class="val muted"># weeks # days</div></div>
        <div class="baby">👶</div>
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        ...
        st.markdown(bar_svg(counts), unsafe_allow_html=True)
